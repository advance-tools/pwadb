...
import { auditTime, delay, filter, map, mergeMap, scan, shareReplay, switchMap } from 'rxjs/operators';
...
export class TreeDatabase<T extends TableDataType> {
    ...
    buildTree(treeInfo: TreeInformation<T>, parentDoc: PwaDocument<T> | null = null, parentParams = new CustomHttpParams(), parentKey: string | null = null): Observable<TreeNode<T>[]> {
        ...
        const treeNodes = Object.keys(treeInfo).map(key => {
            ...
            possibleParamKeys.forEach(pk => {
                ...
                if (!paramName.includes('--')) {
                    ...
                    currentParams = currentParams.set(paramName, parentParams.get(pk) as string);
                }

            });

            // run callback
            treeInfo[key].hasOwnProperty('onCreationSetup') ? (treeInfo[key]['onCreationSetup'] as (parentDoc: PwaDocument<T> | null, db: Database<T> | ReactiveDatabase<T>, params: HttpParams) => void)(parentDoc, db, currentParams) : db.httpParams = currentParams;
            ...
            return db.dataChange.pipe(
                ...
                map(docs => {
                    ...
                    const obsArr = docs.map(doc => {
                        ...
                        return this.childTreeMap.get(doc) as {item: PwaDocument<T>, children: Observable<TreeNode<any>[]>};
...
export class DynamicTreeFlattener<T, F> {
    ...
    _flattenNode(node: T, level: number): F {
        ...
        return this.flatNodeMap.get(node) as F;
    }
    ...
    addChildrenFlatNode(parentFlatNode: F, resultNodes: F[]): Observable<F[]> {
        ...
        const structuredNode = this.structuredNodeMap.get(parentFlatNode) as T;
